...
reenter:
  ...
  {f=0;printf("Input dimension:    "); scanf("%d",&dim);}
...
/* Set matrix and vector pointers as in mcd.c  */
  ...
  { printf("Choose option. Print 'l' for list.\n");
    ...
    { scanf("%d",&n); seeknln();
      ...
      for (i=1;i<=dim;i++) scanf("%d",vec[n]+i); 
      seeknln();
    }
    ...
    { scanf("%d",&n); seeknln();
      ...
      for (i=1;i<=dim;i++) for (j=1;j<=dim;j++) scanf("%d",mat[n][i]+j);
      seeknln();
    }
