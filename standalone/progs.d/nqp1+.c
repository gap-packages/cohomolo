...
*/
  ...
  { orsp=rsp; optrsp=ptrsp; adn=0; ip=fopen(inf3,"r");
    if (ip==0) printf("Warning. File %s is not present.\n",inf3);
    else
    { ipm=fopen(inf4,"r");
      if (ipm==0)
...
*/
  ...
  if ((ch1 && nng==0) || (ch1==0 && chpdim==chsdim)) return(-1);
...
/* if exp=0, the corresponding dcrep matrix must be skipped */
  ...
  rsp=rpb-rel+1; if (norm) return(0);
